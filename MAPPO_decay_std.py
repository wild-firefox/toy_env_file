...
class MAPPO: 
    def __init__(self, dim_info, is_continue, actor_lr, critic_lr, horizon, device, trick = None):        
        ...
        self.action_dim = dim_info[agent_x][1]
        
        if self.is_continue:
            self.act_std_min= 0.05#0.05
            self.act_std_init= 0.05#0.4 # 0.4     
    ...
    def select_action(self, obs):
        ...
        for agent_id, obs in obs.items():
            ...
            if self.is_continue: 
                mean = self.agents[agent_id].actor(obs) # 1xact_dim
    ...
    def evaluate_action(self,obs):
        ...
        for agent_id, obs in obs.items():
            obs = torch.as_tensor(obs,dtype=torch.float32).reshape(1, -1).to(self.device)
            if self.is_continue: 
                mean = self.agents[agent_id].actor(obs)
        ...
        for agent_id, agent in self.agents.items():
            ...
            for _ in range(K_epochs): 
                ...
                for index in indexes:
                    # 先更新actor
                    if self.is_continue:  
                        mean = self.agents[agent_id].actor(obs[agent_id][index])
...
def make_dir(env_name,policy_name = 'DQN',trick = None):
    ...
    print('trick:',trick)
    # 确定前缀
    if trick is None or not any(trick.values()) or policy_name =='MAPPO' or policy_name =='MAPPO_decay_std':
...
6 5 600000
7 6基础上改horizon = 512
8 5 改 entropy_coefficient 0  
9 (MAPPO 6)  6基础上改horizon = 1024
== decay_std 
1            6(MAPPO 3)基础上改 decay_std
2 修改std 不是按照策略调整修改 0.1
3 std 0.4 200000 
4 std 0.2 200000 
5 std 0.4 400000
6 std 0.4 200000 200   
7 std 0.4 200000 改成胜率调整std
8 std 0.2 200000 改成胜率调整std
9 std 0.2 200000 改decay_std  
10 9 改 decay_syd 位置下调到policy_learn里
11       10  /2
12   11 horizon 512
13 12上 0.3 0.01
14 12 改上去
15 12 0.3 0.03 + 改了频率
16 之后改成当std变为小于0.1时 由胜率掌控std
17 16 std 0.2
18 std 0.1    
19 0.1 0.05 频率控制
20 0.2 0.05 频率控制 400000
21  0.3 0.03 lr5e-4  400000 200000//12
22  0.2 0.03 lr1e-3  400000 400000//12
23  0.1 0.05 lr1e-3  400000 400000//12 
24 0.1不变
25 0.05 不变          
'''
if __name__ == '__main__':
    ...
    parser.add_argument("--max_episodes", type=int, default=int(200000))
    ...
    parser.add_argument("--entropy_coefficient", type=float, default=0.01) # 0.01
    ...
    # decay_std 参数
    parser.add_argument("--decaystd_freq", type=int, default=int(400000//12)) #200000//12
    # device参数   
    ...
    print(args)
    print('-' * 50)
    args.policy_name += '_'+'decay_std' # decay_std
    ...
    while episode_num < args.max_episodes:
        ...
        if any(done.values()):
            ...
            if  (episode_num + 1) % 100 == 0:
                ...
                #     #policy.act_std = policy.act_std * 0.9
                ...
                #     policy.act_std = torch.full((1,policy.action_dim),policy.act_std_temp)
                ...
                #     print(f'策略提升{policy_up}',policy.act_std[0][0],args.win_rate_adjust) 
                # # 动态调整
                ...
                #         print(f'策略调整{policy_up}adjust',policy.act_std[0][0],args.win_rate_adjust)

                # if policy.act_std_temp <= 0.1:            
                #     if win1_rate >= args.win_rate_adjust :#        
                #         policy_up += 1
                #         policy.act_std_temp = policy.act_std_temp * 0.9
                #         policy.act_std = torch.full((1,policy.action_dim),policy.act_std_temp)
                #         #args.gauss_sigma = args.gauss_sigma * 0.9 
                ...
                #         episode_temp = episode_num
                #         #print(f'策略提升{policy_up}',args.gauss_sigma,args.win_rate_adjust)  
                #         print(f'策略提升{policy_up}',policy.act_std_temp,args.win_rate_adjust) 
                #     # 动态调整
                #     if win1_rate >= 0.10 : #or policy_up > 20: #此时episode_temp才有定义
                #         if episode_num - episode_temp > 10000:
                #             args.win_rate_adjust -=0.01
                ...
                #             episode_temp = episode_num
                #             #print(f'策略调整{policy_up}adjust',args.gauss_sigma,args.win_rate_adjust)
                #             print(f'策略调整{policy_up}adjust',policy.act_std_temp,args.win_rate_adjust)

                writer.add_scalar('win_rate', win_rate, episode_num)
                writer.add_scalar('win1_rate', win1_rate, episode_num) #大胜利
                writer.add_scalar('act_std', policy.act_std_temp, episode_num)
                # 血量显示
            ...
            #     #if episode_num < (args.max_episodes/2):
            #         explr_pct_remaining = max(0, (args.max_episodes) - (episode_num + 1)) / (args.max_episodes)
            #         policy.act_std_temp = policy.act_std_min + (policy.act_std_init - policy.act_std_min) * explr_pct_remaining
            #         policy.act_std = torch.full((1,policy.action_dim),policy.act_std_temp)
